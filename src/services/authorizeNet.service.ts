...
export class AuthorizeNetService {
   ...
   */
  async getHostedPaymentToken(data: { amount: number; planName: string; description?: string }): Promise<{ token: string; hostedUrl: string }> {
  ...
  async chargePayment(data: AuthorizeNetChargeRequest): Promise<AuthorizeNetResponse> {
    const hostname = this.isSandbox ? 'apitest.authorize.net' : 'api.authorize.net';
